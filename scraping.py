...
def mars_hemis(browser):
    ...
    hemis_data = hemis_soup.find_all('div', class_='description')

    # loop through four pages 
    for i in range(4): 
        # find the link to the full image url
        link_img=hemis_data[i].find('a')['href']
        # find the description for each Hemisphere
        Description=hemis_data[i].find('h3').text
